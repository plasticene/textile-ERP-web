...
import { selfFilterMenu } from '@/utils/authority-utils'
...
export default {
  ...
  getters: {
    menuData(state, getters, rootState) {
      if (state.filterMenu) {
        ...
        return selfFilterMenu(deepClone(state.menuData), permissions, roles)
      }
  ...
  mutations: {
    ...
    toggleMenuData(state, model) {
      const newMenuData = state.menuData.map(route => {
        if (model === 'tmp') {
          if (route.meta.isExample) {
            route.meta.invisible = false
          }
          if (route.meta.isComponents) {
