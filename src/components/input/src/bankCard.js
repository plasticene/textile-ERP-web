...
const BANK_CARD_LIST_LENGTH = bankcardList.length
...
function getBankInfoByCardNo(cardNo) {
  for (var i = 0; i < BANK_CARD_LIST_LENGTH; i++) {
    var bankcard = bankcardList[i]
    var patterns = bankcard.patterns
    const patternInfo = patterns.find(pattern => {
      return new RegExp(pattern.reg).test(cardNo)
    })
    if (patternInfo) {
      return {
        ...omit(bankcard, 'patterns'),
        cardTypeName: getCardTypeName(patternInfo['cardType'])
      }
    }
  }
...
export default getBankInfoByCardNo
