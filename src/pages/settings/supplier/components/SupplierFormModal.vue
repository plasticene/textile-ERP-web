...
<script name="SupplierFormModal" setup>
import { saveSupplier, updateSupplier } from '@/services'
...
const handleOk = () => {
  formRef.value.validate(valid => {
    if (valid) {
      ...
      if (opt.value === 'add') {
        saveSupplier(param).then(() => {
          handleCancel(true)
        })
      } else {
        updateSupplier(param).then(() => {
          handleCancel(true)
        })
      }
    } else {
